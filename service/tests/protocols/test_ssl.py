...
def test_get_ssl_when_valid(app):
    ...
    with app.app_context():
        with mock.patch('ssl.create_default_context') as mock_ssl:    
            ...
            mock_ssl.return_value.wrap_socket.return_value.__enter__.return_value.connect.assert_called_with((monitor.target, 443))
            ...
            assert db_ssl_record.success == True
            assert db_ssl_record.authority == "testAuthority"
            assert db_ssl_record.expiry_date == datetime.datetime(2024, 3, 3, 18, 56, 17)
...
def test_get_ssl_when_cert_is_none(app):
    ...
    with app.app_context():
        with mock.patch('ssl.create_default_context') as mock_ssl:    
            mock_ssl.return_value.wrap_socket.return_value.__enter__.return_value.getpeercert.return_value = None
            get_ssl(monitor)
            ...
            db_ssl_record = SslRecord.query.order_by(SslRecord.id.desc()).first()
            assert db_ssl_record.monitor
            assert db_ssl_record.success == False
            assert db_ssl_record.authority == ""
            assert db_ssl_record.expiry_date == None
