...
def test_monitor_http_records(app):
    ...
    with app.app_context():
        ...
        another_monitor = Monitor.create(protocol.id, 60, "Monitor2", 'www.test2.com', False)

        expected_records = []
        for _ in range(10):
            expected_records.append(HttpRecord.create(monitor.id, 20, 200, ""))
        for _ in range(10):
            # Create unrelated records
            HttpRecord.create(another_monitor.id, 20, 200, "")
...
def test_monitor_ssl_records(app):
    ...
    with app.app_context():
        ...
        expected_records = []
        for _ in range(10):
            expected_records.append(SslRecord.create(monitor.id, True, "Example Auth", datetime.datetime.now()))
        for _ in range(10):
            # Create unrelated records
            SslRecord.create(another_monitor.id, True, "Example Auth", datetime.datetime.now())
