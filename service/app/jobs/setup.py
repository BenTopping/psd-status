    ...
    with scheduler.app.app_context():
        ...
        for monitor in monitors:
            if monitor.protocol == http_protocol:
                scheduler.add_job(id=str(monitor.id), func=context_http, args=[monitor], trigger='interval', seconds=monitor.delay)
            elif monitor.protocol == ssl_protocol:
                scheduler.add_job(id=str(monitor.id), func=context_ssl, args=[monitor], trigger='interval', seconds=monitor.delay)
